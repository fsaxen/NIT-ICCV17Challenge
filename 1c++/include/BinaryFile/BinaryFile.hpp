...
{
	// Open file
	inline std::FILE* open(std::string filename, bool readNotWrite)
	{
		if(readNotWrite)
	...
	inline std::FILE* open(const char* filename, bool readNotWrite)
	{
		if(readNotWrite)
	...
	inline bool close(std::FILE* file)
	{
    ...
    template <typename T>
    bool write_one(FILE * file, const T data)
    {
    ...
    inline bool write_one(FILE * file, const cv::Mat & data)
    {
        bool okay = true;

        // What do you want to do if the matrix is empty? I think this information should be saved. You might have another opinion.
        if(data.empty())
        {
            okay &= write_one(file, int32_t(0));
            okay &= write_one(file, int32_t(0));
            okay &= write_one(file, uint32_t(0));
            return okay;
        }
    ...
    inline bool read_one(FILE * file, cv::Mat & data)
    {
	    bool okay = true;
    ...
    inline bool load_binary_model_file(
		    const char * fn,
    ...
    inline bool save_binary_model_file(
		    const char * fn,
		    int iteration, int points,
